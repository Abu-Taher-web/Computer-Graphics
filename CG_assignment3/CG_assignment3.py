...
class Win(GlutWindow):
    ...
    def draw(self):
        ...
        # Sun
        mvp_stack.append(glm.rotate(glm.mat4(1.0), elapsed, glm.vec3(0,1,0)))
        ...
        mvp_stack[-1] = glm.rotate(mvp_stack[-1], elapsed, glm.vec3(0,1,0))
        ...
        mvp_stack[-1] = glm.rotate(mvp_stack[-1], 6 * elapsed, glm.vec3(0,1,0))
        mvp_stack[-1] = glm.scale(mvp_stack[-1], 0.2 * glm.vec3(1, 1, 1))
        mvp_stack[-1] = glm.translate(mvp_stack[-1], glm.vec3(-20, -3, 0))
        self.model_matrix = mvp_stack[-1]
        self.calc_mvp()
        glUniformMatrix4fv(self.context.mvp_location, 1, GL_FALSE, glm.value_ptr(self.context.mvp))
        glUniformMatrix4fv(self.context.m_location, 1, GL_FALSE, glm.value_ptr(self.model_matrix))
        glUniform3f(self.context.color_location, 1, 1, 1)
        glUniform1f(self.context.light_bool_location, 1.0)
        glDrawElements(GL_TRIANGLES, len(ico_inds), GL_UNSIGNED_SHORT, None)
