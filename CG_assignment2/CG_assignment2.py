...
class Win(GlutWindow):
        ...
        def init_shaders(self):
                ...
                print(f'Shader Program: {shader_program}')
        ...
        def init_context_raw(self):
                ...
                print(f'mvp loaction:{self.context.mvp_location}, texture_location:{self.context.texture_location}, texture:{texture}, glid:{self.context.textureGLID}, buffer:{self.context.vertexbuffer}')
		...
		'''
                ...
                glBufferData(GL_ARRAY_BUFFER, len(uv_buffer_data)*4,
                        (GLfloat * len(uv_buffer_data))(*uv_buffer_data), GL_STATIC_DRAW)
		...
		'''
                ...
                # Invert V coords if necessary
                if texture.inversedVCoords:
                    for i in range(1, len(model.texcoords), 2):
                        model.texcoords[i] = 1.0 - model.texcoords[i]
